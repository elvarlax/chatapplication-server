...
public class SocketServerEngine extends GenericThreadedComponent {
    ...
    /**
     * Vector pool for keeping all the ConnectionHandlers in idle state waiting for
     * new socket connections
     */
    Vector<SocketConnectionHandler> connectionHandlingPool;

    /**
     * Vector holding the references to the connection handlers that are occupied by
    ...
    /**
     * Object providing the secure server socket of the ChatApplication Central
     * System
     */
    ...
    /**
     * Method for printing some information about the established (if any) socket
     * connections to the CA socket server. Actually, it checks the occupance pool
     * to see how many connection handlers are assigned to sockets and then it
    ...
    public void printEstablishedSocketInfo() {
        ...
        /**
         * Take a clone of the occupance pool in order not to block it for a long period
         * of time and loose any new incomaing connections
         */
        ...
        if (occupance.size() == 0) {
            SocketServerGUI.getInstance()
                    .appendEvent("[SSEngine]:: There aren't any established client connections to the CA server ("
     ...
     * initialize of the SocketServerEngine component and handles any configuration
     * that needs to be done in the socket server before it is ready for accepting
     * chat-application client connections. Actually, it creates the
     * ConnectionHandling pool and adds the fired Connectionhandlers that are ready
     * for handling any connection attempts.
     * <p>
    ...
    public void initialize() throws ComponentInitException {
        ...
        /**
         * Set the default value of the number of SSLConnectionHandlers waiting in the
         * connectionHandling pool
         */
        ...
        SocketServerGUI.getInstance().appendEvent(
                "[SSEngine]:: ConnectionHandling Pool (" + configManager.getValue("ConnectionHandlers.Number")
                        + ") fired up (" + lotusStat.getCurrentDate() + ")\n");
        ...
        try {
            ...
            SocketServerGUI.getInstance()
                    .appendEvent("[SSEngine]:: Failed upon initialization of the CS socket server -- " + e.getMessage()
                            + " (" + lotusStat.getCurrentDate() + ")\n");
    ...
    /**
     * Method for removing a connection handler reference from the occupance pool
     * holding all the handlers that are currently assigned to a specific
     * connection. This method is called whenever a connection handler finished
     * handling a connection and wants to get back in the ConnectionHandling pool
     * (in idle state) waiting for new connections. At the same time, it must be
     * removed from the occupied handlers.
     * <p>
     * IMPORTANT NOTE It must run in a synchronized block
     *
     * @param sch The identification name of the Connection Handler that must be
     *            removed from the occupance pool
     */
    public void removeConnHandlerOccp(final String sch) {
        /**
         * A SocketConnectionHandler reference used for fetching already running
         * handlers...
         */
    ...
    /**
     * Method for getting the reference to the connectionHandling pool containing
     * all the handlers in idle state. This method is called by a connection handler
     * that was assigned to a socket connection that finished its work and want to
     * go back to idle state and thus add itself to the pool for future work.
     */
    ...
    /**
     * Method for adding a Connection Handler with the specified name to the already
     * existing ConnectionHandling pool. This method is actually called by a
     * Connection Handler which, for some reason, wants to shut down. Also, this
     * method is responsible for removing this Connection Handler reference from the
     * occupance pool since it finished handling a specific connection.
     * <p>
     * IMPORTANT NOTE The addition to the ConnectionHandling pool must be done in a
     * synchronized block.
     *
     * @param handlerName The String identifier of the Connectionhandler to be added
     *                    in the pool
     */
    public void addConnectionHandlerToPool(final String handlerName) {
        ...
        synchronized (connectionHandlingPool) {
            /** Add him in the pool... */
            connectionHandlingPool.addElement(handler);
            SocketServerGUI.getInstance().appendEvent("[SSEngine]:: " + handler.getHandlerIdentifierName()
                    + " terminated...New reference back in the pool (" + lotusStat.getCurrentDate() + ")\n");
        }
    }

    /**
     * The main logic of the SocketServerEngine. This mainly includes the acceptance
     * of various client connections and assigns them to specific Connection
     * Handlers
     */
    public void componentMain() {
        ...
        /**
         * In case of failure during establishment of a connection, type to the log the
         * reason of rejection
         */
        try {
            while (!mustShutdown) {
                /** Format message saying that we are waiting for new connections... */
                SocketServerGUI.getInstance().appendEvent("[SSEngine]:: Waiting for clients on port "
                        + configManager.getValue("Server.PortNumber") + "\n");

                s = ChatApplication_Server.accept();
                ...
                synchronized (connectionHandlingPool) {
                    ...
                    if (connectionHandlingPool.isEmpty()) {
                        ...
                        SocketServerGUI.getInstance().appendEvent("[SSEngine]:: No more ConnHandlers available for ("
                                + s.getRemoteSocketAddress() + ") (" + lotusStat.getCurrentDate() + ")\n");
                    ...
                    /**
                     * Also put the reference of this occupied connection handler to the
                     * corresponding pool...
                     */
                    synchronized (connHandlerOccp) {
            ...
            /**
             * This kind of exception is thrown whenever the socket server is shutting
             * down;so close the socket blocked
             */
            try {
                ...
                /**
                 * This exception is thrown whenever the blocked socket (waiting for new
                 * connections) could not be shut down...
                 */
                SocketServerGUI.getInstance()
                        .appendEvent("[SSEngine]:: Failed shutting down blocked socket connection --" + ex.getMessage()
                                + " (" + lotusStat.getCurrentDate() + ")\n");
            }
        } catch (final Exception e) {
            /**
             * This exception is thrown whenever (for some reason) the incoming connection
             * could not be established...
             */
            SocketServerGUI.getInstance()
                    .appendEvent("[SSEngine]:: Failed establishing Connection (" + s.getRemoteSocketAddress() + ") -- "
     ...
     * the SocketServerEngine thread, holding the secure socket server, stops.
     * First, we have to stop all the connection handlers running in an idle state
    ...
    public void shutdown() {
        ...
        try {
            synchronized (ChatApplication_Server) {
                 ...
                 * Socket Server...
                 */
                SocketServerGUI.getInstance().appendEvent("[SSEngine]:: Shutting down the Socket Server....COMPLETE ("
                        + lotusStat.getCurrentDate() + ")\n");
            }
        } catch (final Exception e) {
            /**
             ...
             * server failed
             */
            SocketServerGUI.getInstance().appendEvent("[SSEngine]: Failed shutting down CS socket server -- "
                    + e.getMessage() + " (" + lotusStat.getCurrentDate() + ")\n");
        }
