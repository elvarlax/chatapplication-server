...
public class SocketConnectionHandler implements Runnable {
    ...
    /**
     * String identifier of this ConnectionHandler thread (since we have more than 1
     * in the ConnectionHandling pool)
     */
    ...
    /**
     * Method for printing some information about the socket connection that this
     * ConnectionHandler thread is accommodating.
     */
        ...
        if (handleConnection != null) {
            ...
            if (!handleConnection.isClosed()) {
                ...
                SocketServerGUI.getInstance().appendEvent("\n----------[" + handlerName
                        + "]:: Configuration properties of assigned socket connection----------\n");
                SocketServerGUI.getInstance()
                        .appendEvent("Remote Address:= " + handleConnection.getInetAddress().toString() + "\n");
                ...
                SocketServerGUI.getInstance().appendEvent(
                        "Local Socket Address:= " + handleConnection.getLocalSocketAddress().toString() + "\n");
            }
        }
    }

    /**
     * Method for setting the socket connection that this SocketConnectionHandler
     * thread object will handle. We must also set the stream reader/writer of the
     * assigned socket connection. Finally, we must notify it to wake up;as it was
     * in an idle state (in the ConnectionHandling pool) waiting for a new
     * connection to be assigned.
     * <p>
     * IMPORTANT NOTE It must run in a synchronized block
     *
     * @param s A reference to the newly established socket connection that this
     *          SocketConnectionHandler will handle
     */
    synchronized void setSocketConnection(Socket s) {
        ...
        /**
         * Print to the logging stream that this SSLConnectionHandler is assigned to
         * this socket connection...
         */
        SocketServerGUI.getInstance().appendEvent("[SSEngine]:: " + handlerName + " assigned to socket ("
                + handleConnection.getRemoteSocketAddress() + ") (" + connectionStat.getCurrentDate() + ")\n");

        /**
         * If the socket's stream writer/reader are set up correctly...then notify the
         * thread to start working
         */
    ...
    /**
     * Method for setting up the stream reader/writer of the assigned to us secure
     * socket connection between the chat clients and the server.
     *
    ...
    public boolean setSocketStreamReaderWriter() {
        try {
            ...
            SocketServerGUI.getInstance()
                    .appendEvent("[" + handlerName + "]:: Stream corrupted excp during stream reader/writer init -- "
                            + sce.getMessage() + " (" + connectionStat.getCurrentDate() + ")\n");
            ...
            SocketServerGUI.getInstance()
                    .appendEvent("[" + handlerName + "]:: Optional data excp during stream reader/writer init -- "
                            + ode.getMessage() + " (" + connectionStat.getCurrentDate() + ")\n");
     ...
     * Since we will have "ConnectionHandlers.Name" number of thread in the
     * Connectionhandling pool, we must have an identifier for for being able to
     * distinguish them.
     *
    ...
    /*
     * Method for getting the userName of the connected client handled by this
     * thread.
     *
    ...
    /**
     * Java thread entry point... This method contains the main functionality of the
     * SocketConnectionHandler. When the worker handler is in idle state, it just
     * waits to be notified by the SocketServerEngine that a new socket connection
     * has been established and must be handled by this worker. Once a socket
     * connection is assigned to this ConnectionHandler, it waits until there is
     * some data for reception. Upon shut down, it returns to the Connectionhandling
     * pool for future use by another socket connection.
     */
        ...
        while (!mustShutdown) {
            ...
            if (handleConnection == null) {
                try {
                    ...
                    SocketServerGUI.getInstance()
                            .appendEvent("[" + handlerName + "]:: ConnectionHandler in idle state died..."
                                    + e.getMessage() + " (" + connectionStat.getCurrentDate() + ")\n");
            ...
            /**
             * If we are notified/assigned to handle a socket connection... Call the
             * receiveContent method for waiting data/requests from the Alix client. The
             * Connection Handler thread will stay in this method during the lifetime of the
             * assigned socket connection
             */
            if (handleConnection != null && certificateExchangeDone) {
                receiveContent();

                /**
                 * If we finished the 'handling' of the assigned socket connection, add
                 * ourselves in the connectionHandling pool for future use
                 */
    ...
    /**
     * Method for handling any data transmission/reception of the assigned socket
     * connection. The SocketConnectionHandler retrieves the stream sent by the
     * client. Whenever, the stream is empty (client doesn't send anything), the
     * SocketConnectionHandler remains idle and goes back to business only when
     * necessary!!
     */
    public void receiveContent() {
        while (isSocketOpen) {
            try {
                ...
                /** Keep track of this exception in the logging stream... */
                SocketServerGUI.getInstance()
                        .appendEvent(userName + " Exception reading streams(CNFE):" + cnfe.getMessage() + "\n");
                certificateExchangeDone = false;
                isSocketOpen = false;
            } catch (OptionalDataException ode) {
                /** Keep track of this exception in the logging stream... */
                SocketServerGUI.getInstance()
                        .appendEvent(userName + " Exception reading streams(ODE):" + ode.getMessage() + "\n");
                isSocketOpen = false;
                certificateExchangeDone = false;
            } catch (IOException e) {
                /** Keep track of this exception in the logging stream... */
                SocketServerGUI.getInstance()
                        .appendEvent(userName + " Exception reading streams(IOE):" + e.getMessage() + "\n");
    ...
    /**
     * Method that is called whenever a ConnectionHandler thread finished the
     * execution of an assigned socket connection. In that case, it must add itself
     * in the Connectionhandling pool of the SocketServerEngine component for future
     * use and return to idle state waiting for new connections.
     */
    public void socketConnectionHandlerRelease() {
         ...
         */
        SocketServerGUI.getInstance().appendEvent("[" + handlerName + "]:: Finished SckHandling -- Back in the pool ("
                + connectionStat.getCurrentDate() + ")\n");

        /**
         * Get the connectionHandling pool from the SSLEngineServer component to add
         * ourselves
         */
    ...
    /**
     * Method for notifying this SocketConnectionHandler thread to stop its
     * execution.
     */
    public void stop() {
        synchronized (this) {
