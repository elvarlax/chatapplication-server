...
/**
 * The ConfigManager components manages configuration values in the format [key,
 * value]. This component loads config values given by the user during the
 * execution of the ChatApplicationServer system.
 *
 * @author atgianne
 */
public class ConfigManager implements IComponent {
    ...
    /**
     * Implementation of the static getInstance() method. It ensures that only one
     * instance of the ConfigManages component is active at any time.
     */
    ...
    /**
     * Method for getting a config value-as long integer- from the map given its
     * key.
     *
     * @param key The key to use for looking up the value.
    ...
    /**
     * Set a config value from the map given its key. The method will overwrite any
     * existing value already stored in the map with the given key.
     *
     * @param key   The key to use for looking up the value.
    ...
    /**
     * If the given key does not already exist in the map, set it up with the given
     * default key. This method is used by components to establish default values
     * for the initial run of the application.
     *
     * @param key   The key to use for looking up the value.
     * @param value The value to store in the map.
     */
